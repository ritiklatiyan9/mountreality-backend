...
/*
 * Seeds forty historically plausible BOOKED customers for local/demo use.
 * It only touches the named company-owned plots at Diwan Valley and is safe
 * to run again: a completed seed exits without adding duplicate rows.
 *
 * Run a reversible validation first:
 *   node src/scripts/seedBookedCustomerDemo.js --dry-run
 * Then seed:
 *   node src/scripts/seedBookedCustomerDemo.js
 */

const SITE_ID = 5;
const ORGANIZATION_ID = 6;
const SEED_KEY = 'DEMO_BOOKED_CUSTOMERS_2023_2026';
const DRY_RUN = process.argv.includes('--dry-run');
const RECEIPT_COUNT = 16;
const LAST_RECEIPT_DATE = '2026-08-05';

const customers = [
  ['A14', 'Harsh Vardhan', 'Business consultant', '2023-02-07', 0.74],
  ['A15', 'Ishita Mehra', 'Interior designer', '2023-03-21', 0.68],
  ['A17', 'Kunal Sethi', 'Retail entrepreneur', '2023-04-16', 0.81],
  ['A18', 'Tanvi Kapoor', 'HR director', '2023-05-29', 0.72],
  ['A19', 'Rahul Anand', 'Civil engineer', '2023-06-25', 0.88],
  ['A20', 'Divya Bansal', 'Pharmacist', '2023-07-19', 0.63],
  ['A22', 'Nitin Khurana', 'Wholesale trader', '2023-08-11', 0.79],
  ['A23', 'Shreya Arora', 'Marketing manager', '2023-09-14', 0.71],
  ['A24', 'Amit Wadhwa', 'Logistics owner', '2023-10-22', 0.84],
  ['A25', 'Ritika Saini', 'Dentist', '2023-11-18', 0.66],
  ['A27', 'Saurabh Malhotra', 'Technology lead', '2023-12-09', 0.77],
  ['A28', 'Neha Batra', 'Boutique owner', '2024-01-27', 0.69],
  ['A29', 'Pankaj Chawla', 'Insurance advisor', '2024-02-15', 0.86],
  ['A30', 'Anjali Sood', 'Professor', '2024-03-12', 0.73],
  ['A32', 'Gaurav Tandon', 'Restaurant owner', '2024-04-08', 0.62],
  ['A33', 'Priyanka Nayar', 'Chartered accountant', '2024-05-24', 0.82],
  ['A34', 'Varun Grover', 'Manufacturer', '2024-06-17', 0.75],
  ['A35', 'Sonal Jain', 'Lawyer', '2024-07-13', 0.67],
  ['A37', 'Arjun Bedi', 'Sales director', '2024-08-25', 0.83],
  ['A38', 'Mansi Chopra', 'Hotel manager', '2024-09-16', 0.7],
  ['A39', 'Deepak Kohli', 'Contractor', '2024-10-20', 0.89],
  ['A40', 'Kritika Verma', 'Product manager', '2024-11-07', 0.65],
  ['A42', 'Rishabh Arora', 'Financial planner', '2024-12-12', 0.78],
  ['A43', 'Nandini Goyal', 'School owner', '2025-01-18', 0.72],
  ['A44', 'Mohit Bhasin', 'Automobile dealer', '2025-02-09', 0.85],
  ['A45', 'Aarti Kapoor', 'Freelance writer', '2025-02-28', 0.61],
  ['B2', 'Siddharth Kaul', 'Export manager', '2025-03-19', 0.76],
  ['B3', 'Pallavi Singh', 'Architect', '2025-04-11', 0.68],
  ['B4', 'Naveen Walia', 'Business owner', '2025-05-04', 0.81],
  ['B5', 'Rhea Khanna', 'Investment analyst', '2025-05-28', 0.64],
  ['B7', 'Aakash Sharma', 'E-commerce founder', '2025-06-09', 0.73],
  ['B8', 'Nikita Ahuja', 'Clinical psychologist', '2025-07-02', 0.7],
  ['B9', 'Yash Bhatia', 'Textile trader', '2025-07-21', 0.87],
  ['B10', 'Komal Sachdeva', 'Bank officer', '2025-08-08', 0.66],
  ['B12', 'Rohan Goyal', 'Project manager', '2025-08-29', 0.8],
  ['B13', 'Sakshi Bedi', 'Fashion entrepreneur', '2025-09-13', 0.69],
  ['B14', 'Tarun Arora', 'Real-estate consultant', '2025-10-06', 0.84],
  ['B15', 'Isha Dhingra', 'Nutritionist', '2025-10-24', 0.63],
  ['B17', 'Karan Sehgal', 'Transport operator', '2025-11-12', 0.77],
  ['B18', 'Reema Suri', 'Corporate trainer', '2025-12-05', 0.71],
].map(([plotNo, name, occupation, bookingDate, collectedPercent], index) => ({
  plotNo,
  code: plotNo,
  name,
  occupation,
  ...
  collectedPercent,
  stage: 'BOOKED',
  phone: `982220${String(3401 + index).padStart(4, '0')}`,
  avatarIndex: (index % 8) + 1,
}));
...
const memberPhotoUrl = (index) => `http://localhost:${process.env.PORT || 8000}/uploads/members/demo-clients/portrait-${pad(index)}.png`;
...
async function seed() {
  ...
  try {
    ...
    const { rowCount: priorCustomerPhotos } = await client.query(
      `UPDATE members
          SET photo=$1 || LPAD((((id - 1) % 8) + 1)::text,2,'0') || '.png', updated_at=$2
        WHERE site_id=$3 AND notes LIKE $4`,
      [
        `http://localhost:${process.env.PORT || 8000}/uploads/members/demo-clients/portrait-`,
        atNoon(LAST_RECEIPT_DATE),
        SITE_ID,
        'DEMO_CUSTOMER_LIFECYCLE_2023_2026:MEMBER:%',
      ],
    );
    ...
    const allPeople = [
      ...customers.map(({ code, name, phone, occupation, avatarIndex }) => ({ code, name, phone, occupation, avatarIndex, relation: null })),
    ...
    for (const person of allPeople) {
      ...
      const { rows } = await client.query(
        `INSERT INTO members
           (site_id,member_type,full_name,phone,email,address,city,state,pincode,occupation,status,photo,notes,created_by,created_at,updated_at)
         VALUES ($1,'CLIENT',$2,$3,$4,$5,'Gurugram','Haryana','122001',$6,'ACTIVE',$7,$8,$9,$10,$10)
         RETURNING id`,
        [
          SITE_ID,
          person.name,
          ...
          memberPhotoUrl(person.avatarIndex || 1),
          marker,
          adminId,
          atNoon('2023-01-01'),
        ],
