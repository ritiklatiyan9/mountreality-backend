...
export const verifyPayment = asyncHandler(async (req, res) => {
  ...
  const { rows: pending } = await pool.query(
    `SELECT id, billing_cycle FROM subscriptions
     WHERE razorpay_order_id = $1 AND organization_id = $2 AND status = 'pending'
     LIMIT 1`,
    [razorpay_order_id, orgId]
  );
  if (!pending[0]) return res.status(404).json({ message: 'No pending subscription found for this payment' });

  const billingCycle = normalizeBillingCycle(pending[0].billing_cycle);
    ...
    `UPDATE subscriptions
     ...
     FROM (
       SELECT GREATEST(NOW(), COALESCE(
         (SELECT MAX(current_period_end) FROM subscriptions WHERE organization_id = $3 AND status = 'active'),
         NOW()
       )) AS start
     ) AS periods
